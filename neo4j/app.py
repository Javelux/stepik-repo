...
def get_employees(tx, name=None, role=None, sort=None):
    ...
    employees = [{"name": result['e']['name'], "role": result['e']['role'], "id": result['id']} for result in results]
    return employees
...
def add_employee_route():
    ...
    with driver.session() as session:
        check_query = f"MATCH (e:Employee) WHERE e.name = $name RETURN COUNT(e) as count"
        check_result = session.run(check_query, name=name).single()
        if check_result["count"] > 0:
            return jsonify({"error": "Employee already exists."}), 400
        add_query = "CREATE (e:Employee {name: $name, role: $role, department: $department})"
        session.run(add_query, name=name, role=role, department=department)
...
def update_employee_route(employee_id):
    ...
    department = request.json.get("department")
    with driver.session() as session:
        result = session.write_transaction(update_employee, employee_id, name, role, department)
        if "error" in result:
